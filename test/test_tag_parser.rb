...
class TagParserTest < Test::Unit::TestCase
  ...
  def test_parse_katakana
    # JstreamはUTF-8入力も受け付ける
    str = "テスト！あいうえお\r\n"
    strio = StringIO.new(str)
    stream = Jstream.new(strio)
    command, _raw = Aozora2Html::TagParser.new(stream, '！', {}, [], gaiji_dir: nil).process
    expected = 'テスト'
  ...
  def test_parse_gaiji
    ...
    strio = StringIO.new(str)
    stream = Jstream.new(strio)
    command, _raw = Aozora2Html::TagParser.new(stream, '…', {}, [], gaiji_dir: 'g_dir/').process
  ...
  def test_parse_gaiji_jisx0213
    ...
    strio = StringIO.new(str)
    stream = Jstream.new(strio)
    command, _raw = Aozora2Html::TagParser.new(stream, '…', {}, [], gaiji_dir: 'g_dir/', use_jisx0213: true).process
    expected = '&#x6318;'
    assert_equal expected, command.to_s
  end
