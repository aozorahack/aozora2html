...
class EmbedGaijiTagTest < Test::Unit::TestCase
  ...
  def test_gaiji_new
    ...
    assert_equal '<img src="g_dir/foo/1-2-3.png" alt="※(name)" class="gaiji" />', egt.to_s
  end

  def test_unembed_gaiji_new
    egt = Aozora2Html::Tag::UnEmbedGaiji.new(nil, 'テストtest')
    assert_equal '<span class="notes">［テストtest］</span>', egt.to_s
  end
