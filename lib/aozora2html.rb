...
class Aozora2Html
  ...
  KU = '／'.freeze        # U+FF0F 全角スラッシュ
  NOJI = '＼'.freeze      # U+FF3C 全角バックスラッシュ
  DAKUTEN = '″'.freeze   # U+2033 ダブルプライム

  GAIJI_MARK = '※'.freeze
  IGETA_MARK = '＃'.freeze
  RUBY_BEGIN_MARK = '《'.freeze
  RUBY_END_MARK = '》'.freeze
  PAREN_BEGIN_MARK = '（'.freeze
  PAREN_END_MARK = '）'.freeze
  SIZE_SMALL = '小'.freeze
  SIZE_MIDDLE = '中'.freeze
  SIZE_LARGE = '大'.freeze
  TEIHON_MARK = '底本：'.freeze
  COMMAND_BEGIN = '［'.freeze
  COMMAND_END = '］'.freeze
  ACCENT_BEGIN = '〔'.freeze
  ACCENT_END = '〕'.freeze
  AOZORABUNKO = '青空文庫'.freeze
  # PAT_EDITOR = /[校訂|編|編集|編集校訂|校訂編集]$/
  PAT_EDITOR = '(校訂|編|編集)$'.freeze
  PAT_HENYAKU = '編訳$'.freeze
  PAT_TRANSLATOR = '訳$'.freeze
  RUBY_PREFIX = '｜'.freeze
  PAT_RUBY = /《.*?》/.freeze
  PAT_DIRECTION = '(右|左|上|下)に(.*)'.freeze
  PAT_REF = '^「.+」'.freeze
  CHUUKI_COMMAND = '注記付き'.freeze
  TCY_COMMAND = '縦中横'.freeze
  KEIGAKOMI_COMMAND = '罫囲み'.freeze
  YOKOGUMI_COMMAND = '横組み'.freeze
  CAPTION_COMMAND = 'キャプション'.freeze
  WARIGAKI_COMMAND = '割書'.freeze
  KAERITEN_COMMAND = '返り点'.freeze
  KUNTEN_OKURIGANA_COMMAND = '訓点送り仮名'.freeze
  MIDASHI_COMMAND = '見出し'.freeze
  OMIDASHI_COMMAND = '大見出し'.freeze
  NAKAMIDASHI_COMMAND = '中見出し'.freeze
  KOMIDASHI_COMMAND = '小見出し'.freeze
  DOGYO_OMIDASHI_COMMAND = '同行大見出し'.freeze
  DOGYO_NAKAMIDASHI_COMMAND = '同行中見出し'.freeze
  DOGYO_KOMIDASHI_COMMAND = '同行小見出し'.freeze
  MADO_OMIDASHI_COMMAND = '窓大見出し'.freeze
  MADO_NAKAMIDASHI_COMMAND = '窓中見出し'.freeze
  MADO_KOMIDASHI_COMMAND = '窓小見出し'.freeze
  LEFT_MARK = '左'.freeze
  UNDER_MARK = '下'.freeze
  OVER_MARK = '上'.freeze
  MAIN_MARK = '本文'.freeze
  ...
  TEN_MARK = '点'.freeze
  SEN_MARK = '線'.freeze
  OPEN_MARK = 'ここから'.freeze
  CLOSE_MARK = 'ここで'.freeze
  MADE_MARK = 'まで'.freeze
  DOGYO_MARK = '同行'.freeze
  MADO_MARK = '窓'.freeze
  JIAGE_COMMAND = '字上げ'.freeze
  JISAGE_COMMAND = '字下げ'.freeze
  PHOTO_COMMAND = '写真'.freeze
  ORIKAESHI_COMMAND = '折り返して'.freeze
  ONELINE_COMMAND = 'この行'.freeze
  NON_0213_GAIJI = '非0213外字'.freeze
  WARICHU_COMMAND = '割り注'.freeze
  TENTSUKI_COMMAND = '天付き'.freeze
  PAT_REST_NOTES = '(左|下)に「(.*)」の(ルビ|注記|傍記)'.freeze
  PAT_KUTEN = /「※」[は|の]/.freeze
  PAT_KUTEN_DUAL = '※.*※'.freeze
  PAT_GAIJI = '(?:＃)(.*)(?:、)(.*)'.freeze
  PAT_KAERITEN = '^([一二三四五六七八九十レ上中下甲乙丙丁天地人]+)$'.freeze
  PAT_OKURIGANA = '^（(.+)）$'.freeze
  PAT_REMOVE_OKURIGANA = /[（）]/.freeze
  PAT_CHITSUKI = /(地付き|字上げ)(終わり)*$/.freeze
  PAT_ORIKAESHI_JISAGE = '折り返して(\\d*)字下げ'.freeze
  PAT_ORIKAESHI_JISAGE2 = '(\\d*)字下げ、折り返して(\\d*)字下げ'.freeze
  PAT_JI_LEN = '([0-9]+)字'.freeze
  PAT_INLINE_RUBY = '「(.*)」の注記付き'.freeze
  PAT_IMAGE = '(.*)（(fig.+\\.png)(、横([0-9]+)×縦([0-9]+))*）入る'.freeze
  PAT_FRONTREF = '「([^「」]*(?:「.+」)*[^「」]*)」[にはの](「.+」の)*(.+)'.freeze
  PAT_RUBY_DIR = '(左|下)に「([^」]*)」の(ルビ|注記)'.freeze
  PAT_CHUUKI = /「(.+?)」の注記/.freeze
  PAT_BOUKI = /「(.)」の傍記/.freeze
  PAT_CHARSIZE = /(.*)段階(..)な文字/.freeze

  REGEX_HIRAGANA = /[ぁ-んゝゞ]/.freeze
  REGEX_KATAKANA = /[ァ-ンーヽヾヴ]/.freeze
  REGEX_ZENKAKU = /[０-９Ａ-Ｚａ-ｚΑ-Ωα-ωА-Яа-я−＆'，．]/.freeze
  REGEX_HANKAKU = /[A-Za-z0-9#\-&',]/.freeze
  # CJK統合漢字 (U+4E00-U+9FFF) + 特殊文字
  REGEX_KANJI = /[\u4E00-\u9FFF々※仝〆〇ヶ]/.freeze

  KANJI_NUMS = '〇一二三四五六七八九'.freeze
  KANJI_TEN = '十'.freeze
  ZENKAKU_NUMS = '０１２３４５６７８９'.freeze
